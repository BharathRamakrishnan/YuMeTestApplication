//
//  YuMeSdkUnitTestAbortDownload.m
//  YuMeiOSTestApp
//
...
#import "YuMeSdkUnitTestAbortDownload.h"
#import "YuMeSdkUnitUtils.h"

@implementation YuMeSdkUnitTestAbortDownload
...
- (void)abortEventListener:(NSArray *)userInfo {
...
/* Expected behavior */
// 1. Throws Exception: "YuMeSDK_AbortDownload(): YuMe SDK is not Initialized."
- (void)testCase_AbortDownload001 {
    ...
    GHAssertFalse([yumeSDK yumeSdkAbortDownload:&pError], @"Initialization is not Successful.");
    if (pError) {
...
/* Expected behavior */
// 1. Does nothing.
- (void)testCase_AbortDownload002 {
    
    [testCaseResult setObject:NSStringFromSelector(_cmd) forKey:@"testCaseId"];
    [testCaseResult setObject:@"SDK State: Not Initialized" forKey:@"preCondition"];
    [testCaseResult setObject:@"No asset downloads in progress (OR) No paused asset downloads." forKey:@"useCase"];
    ...
    GHTestLog(@"yumeSdkAbortDownload called.");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        ...
        NSString *str = @"yumeSdkAbortDownload Successful.";
        [testCaseResult setObject:str forKey:@"status"];
        [testCaseResult setObject:@"Pass" forKey:@"result"];
        [resultArray addObject:testCaseResult];
    }
...
// 3. Sets the download status to NOT_IN_PROGRESS.
- (void)testCase_AbortDownload003 {
    ...
    [self performSelector:@selector(abortEventListener:) withObject:userInfo afterDelay:0.1];
...
// 2. Cleans-up the partly downloaded files.
// 3. Sets the download status to NOT_IN_PROGRESS.
- (void)testCase_AbortDownload004 {
...
#if kTEST_REPORT
    [YuMeSdkUnitUtils createTestReport:resultArray title:@"Abort Download"];
#endif
