//
//  YuMeSdkUnitTestResumeDownload.m
//  YuMeiOSTestApp
//
...
#import "YuMeSdkUnitTestResumeDownload.h"
#import "YuMeSdkUnitUtils.h"

@implementation YuMeSdkUnitTestResumeDownload
...
- (void)resumeEventListener:(NSArray *)userInfo {
...
/* Expected behavior */
// 1. Throws Exception: "YuMeSDK_ResumeDownload(): YuMe SDK is not Initialized."
- (void)testCase_ResumeDownload001 {
    ...
    GHAssertFalse([yumeSDK yumeSdkResumeDownload:&pError], @"Initialization is not Successful.");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        GHAssertEqualObjects(str, @"yumeSdkResumeDownload(): YuMe SDK is not Initialized.", @"");
        ...
        NSString *str = @"Throws Exception Failed: yumeSdkResumeDownload(): YuMe SDK is not Initialized.";
        [testCaseResult setObject:str forKey:@"status"];
...
/* Expected behavior */
// 1. Does nothing.
- (void)testCase_ResumeDownload002 {
    
    [testCaseResult setObject:NSStringFromSelector(_cmd) forKey:@"testCaseId"];
    [testCaseResult setObject:@"SDK State: Not Initialized" forKey:@"preCondition"];
    [testCaseResult setObject:@"No paused asset downloads." forKey:@"useCase"];
    ...
    pError = nil;
    GHAssertTrue([yumeSDK yumeSdkResumeDownload:&pError], @"yumeSdkResumeDownload Successful.");
    GHTestLog(@"yumeSdkResumeDownload called.");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        ...
        NSString *str = @"yumeSdkResumeDownload Successful.";
        [testCaseResult setObject:str forKey:@"status"];
        [testCaseResult setObject:@"Pass" forKey:@"result"];
        [resultArray addObject:testCaseResult];
    }
...
- (void)testCase_ResumeDownload003 {
    ...
    [self performSelector:@selector(resumeEventListener:) withObject:userInfo afterDelay:0.1];
    ...
    GHTestLog(@"downloadStatus : %u", downloadStatus);
    GHAssertEquals(downloadStatus, 1, @"In Progress Failed");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        [testCaseResult setObject:str forKey:@"status"];
        [testCaseResult setObject:@"Fail" forKey:@"result"];
...
#if kTEST_REPORT
    [YuMeSdkUnitUtils createTestReport:resultArray title:@"Resume Download"];
#endif
