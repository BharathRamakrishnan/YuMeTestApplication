...
#import "YuMeSdkUnitTestIsCache.h"
#import "YuMeSdkUnitUtils.h"

@implementation YuMeSdkUnitTestIsCache
...
/* Expected behavior */
// 1. Throws Exception: "YuMeSDK_IsCacheEnabled(): YuMe SDK is not Initialized."
- (void)testCase_IsCacheEnabled001 {
    ...
    GHAssertFalse([yumeSDK yumeSdkIsCacheEnabled:&pError], @"Initialization is not Successful.");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        GHAssertEqualObjects(str, @"yumeSdkIsCacheEnabled(): YuMe SDK is not Initialized.", @"");
        ...
        NSString *str = @"Throws Exception Problem : yumeSdkIsCacheEnabled(): YuMe SDK is not Initialized.";
        [testCaseResult setObject:str forKey:@"status"];
...
/* Expected behavior */
// 1. Returns true, if cache is enabled.
// 2. Returns false, if cache is disabled.
- (void)testCase_IsCacheEnabled002 {
    ...
    GHAssertTrue(params.bEnableCaching == [yumeSDK yumeSdkIsCacheEnabled:&pError], @"");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        GHTestLog(@"Result : %@", str);
        [testCaseResult setObject:str forKey:@"status"];
        ...
        [testCaseResult setObject:@"yumeSdkIsCacheEnabled Successful." forKey:@"status"];
        [testCaseResult setObject:@"Pass" forKey:@"result"];
        [resultArray addObject:testCaseResult];
    }
    
#if kTEST_REPORT
    [YuMeSdkUnitUtils createTestReport:resultArray title:@"IsCacheEnabled"];
#endif
