//
//  YuMeSdkUnitTestIsAutoPrefetch.m
//  YuMeiOSTestApp
//
...
#import "YuMeSdkUnitTestIsAutoPrefetch.h"
#import "YuMeSdkUnitUtils.h"

@implementation YuMeSdkUnitTestIsAutoPrefetch
...
/* Expected behavior */
// 1. Throws Exception: "YuMeSDK_IsAutoPrefetchEnabled(): YuMe SDK is not Initialized."
- (void)testCase_IsAutoPrefetchEnabled001 {
    ...
    GHAssertFalse([yumeSDK yumeSdkIsAutoPrefetchEnabled:&pError], @"Initialization is not Successful.");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        GHAssertEqualObjects(str, @"yumeSdkIsAutoPrefetchEnabled(): YuMe SDK is not Initialized.", @"");
        ...
        NSString *str = @"Throws Exception Failed: yumeSdkIsAutoPrefetchEnabled(): YuMe SDK is not Initialized.";
        [testCaseResult setObject:str forKey:@"status"];
...
/* Expected behavior */
// 1. Returns true, if auto prefetch is enabled.
// 2. Returns false, if auto prefetch is disabled.
- (void)testCase_IsAutoPrefetchEnabled002 {
    ...
    GHAssertTrue([yumeSDK yumeSdkIsAutoPrefetchEnabled:&pError], @"");
    if (pError) {
        NSString *str = [YuMeSdkUnitUtils getErrDesc:pError];
        ...
        NSString *str = @"yumeSdkIsAutoPrefetchEnabled Successful.";
        [testCaseResult setObject:str forKey:@"status"];
        [testCaseResult setObject:@"Pass" forKey:@"result"];
        [resultArray addObject:testCaseResult];
    }
...
#if kTEST_REPORT
    [YuMeSdkUnitUtils createTestReport:resultArray title:@"Is Auto-Prefetch Enabled"];
#endif
